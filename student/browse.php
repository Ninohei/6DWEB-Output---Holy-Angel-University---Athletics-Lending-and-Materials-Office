...
$is_favorites_filter = ($category_filter === 'favorites');
...
if ($is_favorites_filter) {
    $where_conditions[] = "EXISTS (SELECT 1 FROM favorites f WHERE f.user_id = ? AND f.equipment_id = e.equipment_id)";
    $params[] = $user_id;
} elseif (ctype_digit($category_filter) && (int)$category_filter > 0) {
    ...
    <div class="main-content">
                ...
                <div class="filters-row">
                    ...
                    <div class="filter-group">
                        ...
                        <select name="category" class="form-control">
                            ...
                            <option value="favorites" <?php echo $is_favorites_filter ? 'selected' : ''; ?>>Favorited</option>
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?php echo $cat['category_id']; ?>" <?php echo $category_filter === (string)$cat['category_id'] ? 'selected' : ''; ?>>
        ...
        <?php else: ?>
            <div class="equipment-grid" id="equipment-grid">
                ...
                ?>
                    <div class="equipment-card" onclick="window.location.href='details.php?id=<?php echo $item['equipment_id']; ?>'" style="cursor: pointer;">
                        ...
                        <div class="card-body">
                            ...
                            <div class="availability">
                                ...
                                <button type="button" class="btn btn-secondary btn-sm favorite-btn <?php echo $item['is_favorited'] ? 'favorited' : ''; ?>"
                                        data-equipment-id="<?php echo $item['equipment_id']; ?>"
                                        onclick="event.stopPropagation();"
                                        aria-label="<?php echo $item['is_favorited'] ? 'Unfavorite' : 'Favorite'; ?>"
                                        title="<?php echo $item['is_favorited'] ? 'Remove from favorites' : 'Add to favorites'; ?>">
